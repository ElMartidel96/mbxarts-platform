/**
 * TEMPORARY DIAGNOSTIC - Inspect exact Redis data for Gift #340
 */
...
export default async function handler(
  ...
  try {
    ...
    console.error('🔍 INSPECTING Gift #340 (tokenId=340, giftId=366)');

    // Read BOTH keys
    const dataByGiftId = await redis.hgetall('gift:detail:366');
    const dataByTokenId = await redis.hgetall('gift:detail:340');

    console.error('📊 Data by giftId 366:', {
    ...
    console.error('📊 Data by tokenId 340:', {
    ...
    return res.status(200).json({
      ...
      analysis: {
        emailLocation: (dataByGiftId as any)?.email_plain || (dataByGiftId as any)?.email_encrypted ? '366' :
                       (dataByTokenId as any)?.email_plain || (dataByTokenId as any)?.email_encrypted ? '340' : 'NONE',
        appointmentLocation: (dataByGiftId as any)?.appointment_date ? '366' :
                            (dataByTokenId as any)?.appointment_date ? '340' : 'NONE',
        educationLocation: (dataByGiftId as any)?.education_score_percentage ? '366' :
                          (dataByTokenId as any)?.education_score_percentage ? '340' : 'NONE',
        problem: 'Email likely stored in gift:detail:340 (tokenId) but backend reads gift:detail:366 (giftId)'
      }
    });
