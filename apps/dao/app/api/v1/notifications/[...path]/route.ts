 ...
 *
 * Proxies requests to the Wallets service (gifts.mbxarts.com) for notifications.
 *
 * Supported paths:
 * - /api/v1/notifications - Get/send notifications
 * - /api/v1/notifications/:id/read - Mark as read
 *
 * @endpoint /api/v1/notifications/[...path]
 *
...
function buildTargetUrl(path: string[], searchParams: URLSearchParams): string {
  ...
  return `${baseUrl}/api/v1/notifications/${pathStr}${query ? `?${query}` : ''}`;
}
      ...
      return NextResponse.json(
        {
          ...
          error: {
            ...
            message: 'Request to Notifications service timed out',
          },
          meta: { requestId },
        },
        {
