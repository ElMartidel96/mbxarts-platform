/**
 * TEMPORARY DIAGNOSTIC - Inspect exact Redis data for Gift #339
 */
...
export default async function handler(
  ...
  try {
    ...
    console.error('🔍 INSPECTING Gift #339 (tokenId=339, giftId=365)');

    // Read BOTH keys
    const dataByGiftId = await redis.hgetall('gift:detail:365');
    const dataByTokenId = await redis.hgetall('gift:detail:339');

    console.error('📊 Data by giftId 365:', {
    ...
    console.error('📊 Data by tokenId 339:', {
    ...
    return res.status(200).json({
      ...
      analysis: {
        emailLocation: (dataByGiftId as any)?.email_plain || (dataByGiftId as any)?.email_encrypted ? '365' :
                       (dataByTokenId as any)?.email_plain || (dataByTokenId as any)?.email_encrypted ? '339' : 'NONE',
        appointmentLocation: (dataByGiftId as any)?.appointment_date ? '365' :
                            (dataByTokenId as any)?.appointment_date ? '339' : 'NONE',
        educationLocation: (dataByGiftId as any)?.education_score_percentage ? '365' :
                          (dataByTokenId as any)?.education_score_percentage ? '339' : 'NONE'
      }
    });
