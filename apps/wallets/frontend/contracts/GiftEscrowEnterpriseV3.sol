...
/**
 * @title GiftEscrow v3.0 Enterprise - Perpetual Mode + Extended Timeframes
 * @dev Secure escrow for temporally-locked NFT gifts with perpetual/infinite mode
 * @author mbxarts.com The Moon in a Box property
 * @author Godez22 (Co-Author)
 *
 * V3 IMPROVEMENTS:
 * - 90-day, 1-year, and perpetual (infinite) timeframe options
 * - Perpetual gifts never expire but creator can cancel after 90-day minimum wait
 * - giftCreatedAt tracking for all gifts (analytics + perpetual cancel logic)
 * - Full backward compatibility with V2 (registerGiftMinted + MINTER_ROLE)
 */
contract GiftEscrowEnterpriseV3 is
    ERC2771Context,
    AccessControl,
    Pausable,
    ReentrancyGuard,
    ...
    IERC1155Receiver
{
    ...
    // V3 perpetual mode
    uint40 public constant PERPETUAL_EXPIRY = type(uint40).max; // 1099511627775
    uint256 public constant PERPETUAL_MIN_WAIT = 7776000; // 90 days before creator can cancel
    ...
    // V3 errors
    error PerpetualWaitNotMet(uint256 giftId, uint256 createdAt, uint256 minCancelTime);
    error NotPerpetualGift(uint256 giftId);
    ...
    // V3: Track creation timestamp for all gifts
    mapping(uint256 => uint40) public giftCreatedAt;
    ...
    // =============================================================================
    // INTERNAL: TIMEFRAME RESOLVER
    // =============================================================================

    /**
     * @dev Resolves timeframe option to (duration, expiresAt) tuple
     * @param timeframe 0=15min, 1=7d, 2=15d, 3=30d, 4=90d, 5=1yr, 6=perpetual
     * @return expiresAt The expiration timestamp (PERPETUAL_EXPIRY for perpetual)
     */
    function _resolveTimeframe(uint256 timeframe) internal view returns (uint40) {
        if (timeframe == 0) return uint40(block.timestamp + FIFTEEN_MINUTES);
        if (timeframe == 1) return uint40(block.timestamp + SEVEN_DAYS);
        if (timeframe == 2) return uint40(block.timestamp + FIFTEEN_DAYS);
        if (timeframe == 3) return uint40(block.timestamp + THIRTY_DAYS);
        if (timeframe == 4) return uint40(block.timestamp + NINETY_DAYS);
        if (timeframe == 5) return uint40(block.timestamp + ONE_YEAR);
        if (timeframe == 6) return PERPETUAL_EXPIRY;
        revert InvalidTimeframe(timeframe);
    }
    ...
    /**
     * @dev ZERO CUSTODY: Register NFT already minted to escrow contract
     ...
     * @param timeframe Timeframe option (0-6)
    ...
    function registerGiftMinted(
        ...
        // Verify NFT is owned by this escrow contract
        ...
        uint40 expiresAt = _resolveTimeframe(timeframe);
        ...
        // V3: Track creation time for all gifts
        giftCreatedAt[giftId] = uint40(block.timestamp);
    ...
    /**
     * @dev LEGACY: Create gift with existing NFT transfer
     */
    function createGift(
        uint256 tokenId,
        address nftContract,
    ...
    function claimGift(
        ...
        // V3: Skip expiration check for perpetual gifts
        if (gift.expiresAt != PERPETUAL_EXPIRY) {
    ...
    function returnExpiredGift(uint256 giftId) external nonReentrant whenNotPaused {
        ...
        // V3: Perpetual gifts cannot be returned via this function
        if (gift.expiresAt == PERPETUAL_EXPIRY) revert NotPerpetualGift(giftId);
    ...
    /**
     * @dev V3: Cancel a perpetual gift after minimum wait period (90 days)
     * @param giftId The gift to cancel
     */
    function cancelPerpetualGift(uint256 giftId) external nonReentrant whenNotPaused {
        Gift storage gift = gifts[giftId];
        ...
        uint40 createdAt = giftCreatedAt[giftId];
        uint256 minCancelTime = uint256(createdAt) + PERPETUAL_MIN_WAIT;
        if (block.timestamp < minCancelTime) {
            revert PerpetualWaitNotMet(giftId, createdAt, minCancelTime);
        }
        ...
        emit PerpetualGiftCancelled(giftId, gift.creator, block.timestamp);
    }

    // =============================================================================
    // EMERGENCY: Admin can cancel perpetual gifts (safety net)
    // =============================================================================

    function emergencyCancelGift(uint256 giftId) external onlyRole(EMERGENCY_ROLE) nonReentrant {
        Gift storage gift = gifts[giftId];
    ...
    function canClaimGift(uint256 giftId) external view returns (bool canClaim, uint256 timeRemaining) {
        ...
        // V3: Perpetual gifts are always claimable
        if (gift.expiresAt == PERPETUAL_EXPIRY) {
    ...
    function isGiftExpired(uint256 giftId) external view returns (bool) {
        // V3: Perpetual gifts never expire
        if (gifts[giftId].expiresAt == PERPETUAL_EXPIRY) return false;
    ...
    /**
     * @dev V3: Check if a gift is perpetual
     */
    function isGiftPerpetual(uint256 giftId) external view returns (bool) {
        return gifts[giftId].expiresAt == PERPETUAL_EXPIRY;
    }
