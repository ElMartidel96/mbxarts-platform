/**
 * 🔀 Escrow Service Proxy
 *
 * Proxies requests to the Wallets service (gifts.mbxarts.com) for escrow operations.
 *
 * Supported paths:
 * - /api/v1/escrow - List/create escrow transactions
 * - /api/v1/escrow/:id - Get escrow details
 * - /api/v1/escrow/:id/claim - Claim escrow
 *
 * @endpoint /api/v1/escrow/[...path]
 *
...
function buildTargetUrl(path: string[], searchParams: URLSearchParams): string {
  ...
  return `${baseUrl}/api/v1/escrow/${pathStr}${query ? `?${query}` : ''}`;
}
...
async function proxyRequest(
  ...
  try {
    ...
    if (internalKey) {
      ...
      forwardHeaders['X-API-Key'] = internalKey;
    }
    ...
    console.error('[Escrow Proxy] Error:', error);
      ...
      return NextResponse.json(
        {
          ...
          error: {
            ...
            message: 'Request to Escrow service timed out',
          },
          meta: { requestId },
        },
        {
    ...
    return NextResponse.json(
      {
        ...
        error: {
          ...
          message: 'Escrow service is temporarily unavailable',
        },
        meta: { requestId },
      },
      {
