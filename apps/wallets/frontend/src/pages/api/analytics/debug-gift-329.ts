/**
 * DEBUG ENDPOINT: Check Redis data for gift #329
 * CRITICAL: User reports tokenId 329 claimed but no data showing
 */
...
export default async function handler(
  ...
  try {
    ...
    const tokenId = '329';
    const giftId = '355'; // From API response
    ...
    results.checks.push({
      ...
      hasEmailEncrypted: !!(giftDetail as any)?.email_encrypted,
      hasEducationData: !!(giftDetail as any)?.education_score_percentage
    });
    ...
    results.checks.push({
      ...
      giftEvents: giftEvents.map(([id, fields]: [string, any]) => ({
        ...
        claimer: fields.claimer || fields.data?.claimer,
    ...
    results.summary = {
      ...
      hasClaimer: results.checks[2].hasClaimer,
      claimerAddress: results.checks[2].claimerValue,
      hasEmailData: results.checks[2].hasEmailEncrypted,
      hasEducation: results.checks[3].exists,
      hasEvents: results.checks[4].giftEventsCount > 0,
      diagnosis: ''
    };

    if (!results.summary.hasGiftDetail) {
      results.summary.diagnosis = 'CRITICAL: No gift:detail found - gift may not exist in Redis';
    } else if (!results.summary.hasClaimer) {
      results.summary.diagnosis = 'ERROR: gift:detail exists but NO claimer field - claim not tracked';
    } else if (!results.summary.hasEmailData) {
      results.summary.diagnosis = 'WARNING: No email data captured during education flow';
    } else if (!results.summary.hasEvents) {
      results.summary.diagnosis = 'WARNING: Claimer exists but no events in stream';
    } else {
